...
margin-left: 180mm;
	...
	<ul class="description">
		...
		<li>上段: 件数</li>
		<li>下段: 金額</li>
		<li><?php echo $total['cnt_total'] ?></li>
...
<?php foreach ($monthly as $month => $value) : ?>
		...
		<?php foreach ($table_keys as $key => $val) : ?>
			<td>
				<?php echo isset($value['cnt'][$key]) ? $value['cnt'][$key] : 0; ?>
			</td>
		<?php endforeach; ?>
			<td><?php echo $value['cnt_total'] ?></td>
		</tr>
...
<?php endforeach; ?>
	...
	<tfoot>
		...
		<?php foreach ($table_keys as $key => $val) : ?>
			<td>
				<?php echo isset($total['cnt'][$key]) ? $total['cnt'][$key] : 0; ?>
			</td>
		<?php endforeach; ?>
			<td><?php echo $total['cnt_total'] ?></td>
		</tr>
		<tr>
			<th>
				合計
