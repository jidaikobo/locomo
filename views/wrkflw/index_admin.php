...
<table class="tbl datatable">
	<thead>
		<tr>
			...
			<th class="al"><?php echo \Pagination::sort('name', 'ワークフロー名'); ?></th>
			<th class="ctrl">操作</th>
		</tr>
	</thead>
	<tbody>
<?php foreach ($items as $item): ?>
		<tr tabindex="-1" title="<?php echo 'ID'.$item->id.' '.$item->name ?>">
			<td tabindex="0"><?php echo $item->id; ?></td>
			<td style="min-width: 6em;" ><div class="col_scrollable"><?php echo $item->name ;?></div></td>
			<td>
				<div class="btn_group">
					<?php
					echo Html::anchor('workflow/workflowadmin/view'.'/'.$item->id, '<span class="skip">ID'.$item->id.' '.$item->name.'を</span>表示', array('class' => 'view'));
					if ( ! $item->deleted_at):
