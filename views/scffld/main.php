...
<p><?php echo \Form::textarea('cmd',
															Input::post('cmd', \Session::get_flash('cmd_raw')),
															array(
																'style' => 'width:100%;height:10em;',
																'placeholder'=>'拡張oilコマンド書式')); ?></p>

<p><?php echo \Form::select('type',
														Input::post('type', \Session::get_flash('type')),
														array(
															'app' => '通常のコントローラ一式として生成する',
															'module' => 'モジュールとしてすべての生成する',
															'model' => 'モデルとマイグレーションのみ生成する',
															'view' => 'viewsとpresenterのみ生成する'
														)); ?></p>

<p><?php echo \Form::select('output',
														Input::post('output', \Session::get_flash('output')),
														array(
															1 => 'output一式を生成する',
															0 => 'output一式を生成しない',
														)); ?></p>

<p><?php echo \Form::select('model',
														Input::post('model', \Session::get_flash('model')),
														array(
															'Model_Base' => '\Orm\Model',
															'Model_Base_Soft' => '\Orm\Model_Soft',
															'Model_Base_Temporal' => '\Orm\Model_Temporal (experiment)',
															'Model_Base_Nestedset' => '\Orm\Model_Nestedset (experiment)'
														)); ?>（モデル生成時のみ）</p>
