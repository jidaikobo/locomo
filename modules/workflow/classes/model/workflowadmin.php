...
class Model_Workflowadmin extends \Locomo\Model_Base
{
	...
	*/
	public static function find_writers($workflow_id = null)
	{
		...
		$q = \DB::select('user_usergroups.user_id');
		$q->from('workflow_allowers');
		$q->join('user_usergroups');
		$q->on('user_usergroups.group_id', '=', 'workflow_allowers.usergroup_id');
		$q->where('workflow_allowers.workflow_id', $workflow_id);
		$q->where('workflow_allowers.is_writer', true);
	...
	*/
	public static function find_allowers($step_id = null)
	{
		...
		$q->join('user_usergroups');
		$q->on('user_usergroups.group_id', '=', 'workflow_allowers.usergroup_id');
		$q->where('workflow_allowers.step_id', '=', $step_id);
		$q->where('workflow_allowers.is_writer', false);
		$allowers = $q->execute()->as_array();
		$allowers = \Arr::flatten($allowers);
