...
(function (factory) {
	...
	var methods = {
		init: function(options)
		{
			return this.each(function()
			{
				...
				//関数で設定値を切り分けたいときに
				if($.type(settings.beforeRender) === "function"){
					settings.beforeRender(self);
				}
		...
		show: function(e)
		{
			...
			if($.type(settings.beforeShow) === "function"){
				settings.beforeShow();
			}
			if (settings.useSelect) {
	...
	function _render(self)
	{
		...
		//settings.stepの設定がなく、かつminで始まるクラスが付与されていれば、その値をsetting.stepに反映。
		if(settings.step==30){
			var class_str, arr, str, min; 
			class_str = self.attr('class');
			arr = class_str.replace(/\s+/g, ' ').split(' ');
			for(key in arr){
				str = arr[key];
				if(str.indexOf('min') == 0){
					min = parseInt(str.slice(3), 10);
					if(min > 0) settings.step = min;
				}
			}
		}
	...
	*/
	...
	$.fn.timepicker.defaults = {
		...
		beforeRender : null,//とりあえず
		beforeShow : null,//とりあえず
		input: null,
	};
}));
